...
#### Steps
1. Run `node --test tests/auth-persistence.test.mjs`.
2. Let the test start a temporary codexUI instance with password auth.
3. Let the test log in, restart the server, and retry with the same cookie.

#### Expected Results
- The login response includes `Max-Age` and `Expires` on `portal_session`.
- The authenticated app shell loads before restart.
- The authenticated app shell still loads after restart.
- The login form does not reappear after restart.
